...
<div class="container py-6">
	<!-- START MODAL -->
	<h3 class="text-center">Pembayaran Yang Sedang Diproses</h3>
...
<div class="py-2">
	<form action="{{ route('pos.paymentsementara') }}" method="POST" style="display: flex; align-items: center">
		@csrf
		<select data-role="select" name="nama" style="margin: 0 !important;">
			<option value="">Pilih Produk</option>
			@foreach($produks as $produk)
			<option value="{{$produk->nama}}">{{$produk->nama}} || Rp. {{$produk->hargajual}}</option>
			@endforeach
		</select>
		<button type="submit" class="button button-info float-right ml-5">Selesaikan Pembayaran</button>
	</form>
</div>
<table class="table hover table-bordered mt-4">
	<thead>
		<tr>
